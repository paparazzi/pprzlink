...
let parse_class = fun xml_class ->
  ...
  and by_name = Hashtbl.create 13
  and class_id = try Some (int_of_string (Xml.attrib xml_class "id")) with _ -> None in
  List.iter
    (fun xml_msg ->
...
let offset_sender_id = 0
let offset_receiver_id = 1
let offset_component_id = 2
let offset_class_id = 2
let offset_msg_id = 3
let offset_fields = 4
(* class and component IDs are encoded on the same byte *)
let mask_component_id = 0xF0
let shift_component_id = 4
let mask_class_id = 0x0F
let shift_class_id = 0
...
module MessagesOfXml(Class:CLASS_Xml) = struct
  let max_length = 256
  let messages_by_id, messages_by_name, class_id =
    try
  ...
  let value_of_byte = fun byte mask shift -> (byte land mask) lsr shift

  let valid_class_id = fun c ->
    match class_id with
    | None -> true
    | Some x -> x = c
