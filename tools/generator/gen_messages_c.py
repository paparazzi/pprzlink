...
static inline void pprz_send_msg_${msg_name}(struct transport_tx *trans, struct link_device *dev, uint8_t ac_id,${{fields: ${attrib_fun}}}) {
...
#define _PPRZ_${class_name}_RET_float(_payload, _offset) ({ union { uint32_t u; float f; } _f; _f.u = _PPRZ_VAL_RET_uint32_t(_payload, _offset); _f.f; })
...
#define _PPRZ_${class_name}_RET_double(_payload, _offset) ({ union { uint64_t u; double f; } _f; _f.u = (uint64_t)(_PPRZ_VAL_RET_uint64_t(_payload, _offset)); Swap32IfBigEndian(_f.u); _f.f; })
